...
export const canNormalSummon = (gameState: GameState, card: CardInstance): boolean => {
  ...
  if (card.card.card_type === '融合モンスター' || 
      card.card.card_type === 'シンクロモンスター' || 
      card.card.card_type === 'エクシーズモンスター' || 
      card.card.card_type === 'リンクモンスター') return false;
...
export const canActivateSpell = (gameState: GameState, card: Card): boolean => {
  ...
  if (card.card_name === 'ジャック・イン・ザ・ハンド') {
    if (gameState.hasActivatedExtravagance) return false; // 金満で謙虚な壺発動済みなら不可
  ...
  // 手札に加える系のカードは金満で謙虚な壺発動済みなら不可
  const drawEffectCards = ['テラ・フォーミング', 'エマージェンシー・サイバー', '儀式の準備'];
  if (drawEffectCards.includes(card.card_name) && gameState.hasActivatedExtravagance) {
    return false;
  }
...
export const canActivateAdvancedRitual = (gameState: GameState): boolean => {
  ...
  // 手札の儀式モンスターの最小レベルを取得
  const minRitualLevel = Math.min(...ritualMonsters.map(c => {
    const monster = c.card as { level?: number };
...
export const canSetSpellTrap = (gameState: GameState, card: Card): boolean => {
  const settableTypes = [
    '通常魔法', '速攻魔法', '永続魔法', '装備魔法', '儀式魔法',
    '通常罠カード', '永続罠カード', 'カウンター罠カード'
  ];
