import { CommonMonsterMap, COMMON_MONSTERS } from "./commonMonsters";
import { EXTRA_MONSTERS, ExtraMonsterMap } from "./extraMonster";
import { MAGIC_CARDS } from "./magicCard";
import { TRAP_CARDS, TrapCardMap } from "./trapCard";
...
const DECK_CONFIG = {
    ...
    ] satisfies { card_name: (typeof EXTRA_MONSTERS)[number]["card_name"]; quantity: number }[],
    token: [{ card_name: "幻獣機トークン", quantity: 1 }],
};

// Helper function to expand deck list
const expandDeckList = (deckList: { card_name: string; quantity: number }[]): Card[] => {
    const result: Card[] = [];
