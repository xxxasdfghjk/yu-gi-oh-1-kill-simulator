        ...
        onSpell: {
            condition: (state) =>
                ...
                state.graveyard.filter(
                    (e) => monsterFilter(e.card) && e.card.monster_type === "通常モンスター" && e.card.attack <= 1500
                ).length > 0,
            effect: (state, _card, _, resolve) => {
                withUserSelectCard(
                    state,
                    _card,
                    (state) =>
                    ...
                    (state, _card, selectedList) => {
                        ...
                        withDelay(state, _card, { order: -1, delay: selectedList.length * 20 }, (state, card) => {
                            resolve?.(state, card);
                        });
                    }
                );
